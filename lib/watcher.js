...
function watcher(settings) {
  ...
  config.load(settings, function (config) {
    ...
    if (!config.required) {
      ...
      process.on(restartSignal, watcher.restart);
      utils.bus.on('error', () => {
    ...
    if (config.options.stdout === false) {
      watcher.on('start', function () {
        watcher.stdout = bus.stdout;
        watcher.stderr = bus.stderr;
...
watcher.addListener = watcher.on = function (event, handler) {
...
watcher.once = function (event, handler) {
  if (!eventHandlers[event]) { eventHandlers[event] = []; }
  eventHandlers[event].push(handler);
...
watcher.removeAllListeners = function (event) {
